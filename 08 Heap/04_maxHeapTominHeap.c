...
int maxheapify(int n, int i) {
  ...
  if (left < n && heap1[left] > heap1[largest]) largest = left;
  if (right < n && heap1[right] > heap1[largest]) largest = right;
  if (largest != i) {
...
int maxinsert(int n) {
  ...
  heap1[n] = value;
  n++;
  for (j = n / 2 - 1; j >= 0; j--) maxheapify(n, j);
  return n;
}

int minheapify(int n1, int i) {
  int largest = i, left = (2 * i) + 1, right = (2 * i) + 2;
  if (left < n1 && heap2[left] < heap2[largest]) largest = left;
  if (right < n1 && heap2[right] < heap2[largest]) largest = right;
  if (largest != i) {
...
int mininsert(int n1, int value) {
  ...
  heap2[n1] = value;
  n1++;
  for (j = n1 / 2 - 1; j >= 0; j--) minheapify(n1, j);
  return n1;
}
...
int main() {
  ...
  printf("\n\n");
  for (i = 0; i < n1; i++) n2 = mininsert(n2, heap1[i]);
  display2(n2, 0, 0);
}
